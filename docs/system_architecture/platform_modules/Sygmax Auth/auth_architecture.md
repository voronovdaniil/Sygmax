...
## Как работает система регистрации аккаунта на платформе Sygmax Platform через микросервис Sygmax Auth `/auth/register` с обработкой ошибок

1. **Клиент отправляет запрос** `POST /auth/register` с email, паролем и именем.
2. **`Sygmax Auth` проверяет**, существует ли такой email.
3. **Хеширует пароль (`bcrypt`/`argon2`) и создает запись в `users`**.
4. **Назначает пользователя в `user_roles` как `Admin`**.
5. **Отправляет запрос в `Workspace Service`** для создания `Personal` воркспейса.
    - Если ответ **успешный** → сохраняет `default_workspace_id`.
    - Если **ошибка** → ждет **2 секунды** и **повторяет запрос**.
    - Если **повторный запрос тоже проваливается** → **откатывает регистрацию** и удаляет пользователя.
6. **Отправляет `JWT` в ответ**, чтобы пользователь сразу вошел в систему.

---

### Ошибка при создании воркспейса

- Если `Workspace Service` **не отвечает или возвращает ошибку**:
  1. **Ждем 2 секунды**.
  2. **Делаем повторный запрос**.
  3. **Если повторно неудачно → удаляем учетную запись (`Rollback`)**.
  4. **Возвращаем 500 с сообщением об ошибке.**

### **Ответ клиенту при провале регистрации**
```json
{
  "error": "Ошибка при создании аккаунта. Попробуйте позже."
}
```
### 3. Откат регистрации (Rollback)

Если **дважды не удалось создать воркспейс**, то:

1. **Удаляем пользователя из `users`**.
2. **Удаляем его роль из `user_roles`**.
3. **Отправляем клиенту ошибку 500**.
