...
#### Как работает система создания аккаунта в Sygmax Platform через Sygmax Auth
...
5. Отправляет запрос в Workspace Service:
    - Создает воркспейс "Personal".
    - Назначает пользователя Admin этого воркспейса.
    - Workspace Service возвращает workspace_id.
6. Обновляет users.default_workspace_id этим workspace_id.
7. Отправляет JWT в ответ, чтобы пользователь сразу вошел в систему.
